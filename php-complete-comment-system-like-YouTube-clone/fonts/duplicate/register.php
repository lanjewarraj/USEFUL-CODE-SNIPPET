...
elseif ($name == "" || $username == "" || $email == ""){
   ...
   <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Email already exists in the database!</div>";
          ...
          if ($sql) {
              # code...
            echo "Inserted in the database";
          }

          else {
           die("Not Inserted in the database".mysqli_error($con)); 
          }*/
			//end test

			//add ref bonus
			if($refcode !== ""){
		    $con->query("UPDATE users SET earning = earning + 10 WHERE refcode = '$refcode'");
			 }
				...
				Hi .$name.
                    ...
                    <a href='https://bestquotesng.com/PHPEmailConfirmation/confirm.php?email=$email&token=$token'>Click Here</a>
                ";

                if ($mail->send())
                    $msg = "<div class='alert alert-success alert-dismissible'>
   <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>".$name."</strong> you have been registered please verify your email inbox or spam folder!</div>";
                else
                    $msg = "<div class='alert alert-danger alert-dismissible'>
   <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Something wrong happen please try again later!</div>"; 
			}
		}
	}
	}
